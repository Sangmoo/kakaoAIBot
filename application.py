...
# chorong 인스턴스 생성
chorong = Chatbot(model=model.basic, system_role=system_role, instruction=instruction)
...
def format_response(resp, useCallback=False):
    data = {
        ...
        "template": {"outputs": [{"simpleText": {"text": resp}}]},
    }
    return data
...
#     chorong.add_user_message(request_message)
#     response = chorong.send_request()
#     chorong.add_response(response)
#     response_message = chorong.get_response_content()
#     chorong.handle_token_limit(response)
#     chorong.clean_context()
...
#     executor.submit(async_send_request, chorong, request_message, callbackUrl)
...
def chat_kakao():
    ...
    request_message = request.json["userRequest"]["utterance"]
    callbackUrl = request.json["userRequest"]["callbackUrl"]
    # chorong 객체에 사용자 메시지를 미리 넣어 둠
    chorong.add_user_message(request_message)
    # chorong.send_request 메소드가 실행될 미래를 담고 있는 future 객체 반환
    future = executor.submit(chorong.send_request)
    try:
        # chorong.send_request가 종료되면 그 결과를 반환
        # 단, 4초까지 기다리다가 완료가 안되면 concurrent.futures.TimeoutError 예외 발생
        response_from_openai = future.result(timeout=1)
        chorong.add_response(response_from_openai)
        response_to_kakao = format_response(
            chorong.get_response_content(), useCallback=False
        )
        print("4초 내 응답:", response_to_kakao)
        return response_to_kakao
    except concurrent.futures.TimeoutError:
        # 4초가 지난 경우 비동기적으로 응답결과를 보냄.
        # 이때 chorong.send_request의 미래를 담고 있는 future도 함께 넘김
        executor.submit(async_send_request, chorong, callbackUrl, future)
